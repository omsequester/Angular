...
if (strlen(trim($username))>0 && strlen(trim($password))>0 && strlen(trim($email))>0 && $emain_check>0 && $username_check>0 && $password_check>0)
{
	...
	if($mainCount==0)
	{
		...
		$sql1="INSERT INTO users(username,password,email,last_login,email_activation,status)VALUES(:username,:password,:email,:created,:email_activation,:status)";
...
function publicUserDetails() {
	...
	$sql = "SELECT uid,tour,username,name,bio,email,friend_count,profile_pic,conversation_count,updates_count,profile_bg,group_count,profile_pic_status,profile_bg_position,verified,notification_created,photos_count,facebookProfile,twitterProfile,googleProfile,instagramProfile,gender FROM users WHERE uid=:uid AND status='1'";
	try {
...
$data = json_decode($request->getBody());
 ...
 $sql = "SELECT a_title,a_desc,a_url,a_img FROM advertisments WHERE status='1' ORDER BY a_id DESC";
	try {
		$key=md5(SITE_KEY.$data->uid);
        if($key==$data->token)
        {
